...
public class SCE {
	...
	private static Circle makeCircleTwoPoints(List<Point> points, Point p, Point q) {
		...
		for (Point r : points) {
			...
			float cross = (float) pq.cross(r.subtract(p));
			Circle c = makeCircumcircle(p, q, r);
			if (c == null)
				continue;
			else if (cross > 0 && (left == null || pq.cross(c.getPoint().subtract(p)) > pq.cross(left.getPoint().subtract(p))))
				left = c;
			else if (cross < 0 && (right == null || pq.cross(c.getPoint().subtract(p)) < pq.cross(right.getPoint().subtract(p))))
				right = c;
		}
	...
	static Circle makeCircumcircle(Point a, Point b, Point c) {
		...
		if (d == 0)
			return null;
		float x = (float) (((ax * ax + ay * ay) * (by - cy) + (bx * bx + by * by) * (cy - ay)
				+ (cx * cx + cy * cy) * (ay - by)) / d);
