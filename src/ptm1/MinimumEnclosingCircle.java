...
public interface MinimumEnclosingCircle {
    ...
    default float dist(Point a, Point b){
        return (float) Math.sqrt(Math.pow(a.x - b.x, 2)
                + Math.pow(a.y - b.y, 2));
    }

   default boolean isInside(Circle c, Point p) {
        return dist(c.p,p)<= c.radius;
    }

    default Point getCircleCenter(float bx,float by, float cx, float cy){
    ...
    default Circle getCircleFrom(Point a, Point b, Point c){
    ...
    default Circle getCircleFrom(Point a, Point b){
        Point c = new Point((a.x+b.x)/2,(a.y+b.y)/2);

        return new Circle(c, dist(a,b)/2);
    }

    default boolean isValidCircle(Circle c, ArrayList<Point> pArr){
        for(Point p : pArr){
            if(!isInside(c,p))
                return false;
        }
    ...
    default Circle minCircleTrivial(ArrayList<Point> pArr){
